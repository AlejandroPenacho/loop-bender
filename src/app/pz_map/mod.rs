...
pub fn show_pz_map(
    ...
    let pole_plot_points: egui::plot::PlotPoints = model
        .get_poles()
        .iter()
        .chain(pid.get_poles().iter())
        .map(|x| x.as_cartesian())
        .collect();

    let zero_plot_points: egui::plot::PlotPoints = model
        .get_zeros()
        .iter()
        .chain(pid.get_zeros().iter())
        .map(|x| x.as_cartesian())
        .collect();
    ...
    ui.horizontal(|ui| {
        if egui::widgets::Button::new("Move")
            .fill(fill_color[0])
            .ui(ui)
            .clicked()
        {
        ...
        if egui::widgets::Button::new("Add pole")
            .fill(fill_color[1])
            .ui(ui)
            .clicked()
        {
        ...
        if egui::widgets::Button::new("Add zero")
            .fill(fill_color[2])
            .ui(ui)
            .clicked()
        {
        ...
        if egui::widgets::Button::new("Remove")
            .fill(fill_color[3])
            .ui(ui)
            .clicked()
        {
