...
pub struct DiagramsConfiguration {
    ...
    pub show_saved_controller: bool,
...
pub fn show_bode_plots(
    ...
    let all_freqs_expo = (0..500)
        .map(|i| (i as f64 / 100.0) - 3.0)
        .collect::<Vec<f64>>();
    let all_freqs = all_freqs_expo
        .iter()
        .map(|i| 10f64.powf(*i))
        .collect::<Vec<f64>>();
    ...
    let (saved_controller_mags, saved_controller_phase, saved_controller_nyquist) =
        if config.show_saved_controller {
            let (mags, phases, _) = compute_phase_and_margin(&saved_system, &all_freqs);

            let mag_line: egui::plot::PlotPoints =
                (0..500).map(|i| [all_freqs_expo[i], mags[i]]).collect();
            let phase_line: egui::plot::PlotPoints =
                (0..500).map(|i| [all_freqs_expo[i], phases[i]]).collect();
...
fn bode_phase_y_spacer(_grid: egui::plot::GridInput) -> Vec<egui::plot::GridMark> {
...
fn compute_phase_and_margin<T: DynamicalSystem>(
    ...
    let phases = frequencies
        ...
        .map(|f| {
            ...
            if prev_phase.map_or(false, |prev| {
                prev >= -std::f64::consts::PI && phase < -std::f64::consts::PI
            }) && phase_cross_point.is_none()
            {
